 ...
 */
Status copy_bmp_header(FILE *fptr_src_image,FILE *fptr_stego_image)             //184 function call
{
    char str[100];
...
Status encode_magic_string(char* magic_string,EncodeInfo *encInfo)         //188  function calling
{
    int i;
    char arr[8];
	...
	encode_byte_to_lsb(magic_string[i],arr);       //61 fuction defination 
	fwrite(arr,8,1,encInfo->fptr_stego_image);
    }
    return e_success;
}
// encoding data the 1byte of data to  the 8byte of image data
Status encode_byte_to_lsb(char data, char* buffer_image)          //54 function caling
{
    int j=0,i;
    for(i=7;i>=0;i--)
    {
...
Status encode_secret_file_extn_size(int size,EncodeInfo *encInfo)             //193 function calling 
{
    char str[32];
    fread(str,32,1,encInfo->fptr_src_image);
    encode_size_to_lsb(size,str);          //81 ficton defination 
    fwrite(str,32,1,encInfo->fptr_stego_image);
    return e_success;
}
// encoding the size 1byte of data to the 8bytes of output image data
Status encode_size_to_lsb(int data, char* buffer_image)            //76 fuction calling 
{
    int j=0,i=0;
    for(i=31;i>=0;i--)
    {
...
Status encode_secret_file_extn(char *file_extn, EncodeInfo *encInfo)       //196 fuction calling
{
    char str[8];
    for(int i=0;file_extn[i]!=0;i++)
    {
	fread(str,8,1,encInfo->fptr_src_image);
	encode_byte_to_lsb(file_extn[i],str);                  //61 function defination 
...
Status check_capacity(EncodeInfo *encInfo)      //182 function calling
{
    encInfo->image_capacity = get_image_size_for_bmp(encInfo->fptr_src_image);//159 function defination
...
uint get_image_size_for_bmp(FILE *fptr_image)         //142 function calling
{
    uint width, height;
    // Seek to 18th byte
...
{
    if (open_files(encInfo) == e_success)        //277 function defination
    {
	printf("SUCCESS: %s function completed\n", "open_files" );
	if(check_capacity(encInfo) == e_success)              //140 function defination
	{
	    if(copy_bmp_header(encInfo->fptr_src_image,encInfo->fptr_stego_image)== e_success)            //22 function defintion
	    {
		printf("success");
		// call the function -> encode magic string
		encode_magic_string(MAGIC_STRING,encInfo);                    //47 function defination

		// store secret file extn into extn_secret_file array 
		strcpy(encInfo->extn_secret_file,strstr(encInfo->secret_fname,"."));
		int size=strlen(encInfo->extn_secret_file);
		encode_secret_file_extn_size(size,encInfo);          //72 fuction defination    

		// store secrete file extension
		encode_secret_file_extn(encInfo->extn_secret_file, encInfo);              //91 fuction defination
 ...
 */
Status open_files(EncodeInfo *encInfo)      //179 function calling
{
    printf("enter\n");
    // Src Image file
